...
\item	 optional functional parameters: data code (\wo{FID}), network code (\wo{FDSN}), data format (\wo{RAWFORMAT}) and source (\wo{RAWDATA}), time zone (\wo{TZ}), acquisition period and delay;	 
...
In the specific case of instrumental station, a calibration table is attached to the \wo{node} sensors that describes each channel characteristics and history:
...
Some \wo{procs} may need several codes or parameters to process a \wo{node}. It is possible to define specific parameters with the prefix \wo{FID\_}: they will be listed and editable below the main FID. Creation/edit of a \wo{FID\_SUBNAME} is reserved to \wo{users} with administrator level.
...
A \wo{node} may have one (and only one) \wo{Project} description: a free-length text file, accepting Wiki syntax. Project is a special \wo{Event}, see \wo{Events logging} for
a description of Project.

% -----------------------------------------------------------------
\subsection{Events}

A \wo{node} can have events:  free-length text files with timestamp. An event can include one or more sub-events (children).
Event files  accept the Wiki syntax. See \wo{Events logging} section (\ref{events_logging}) for a description of Nodes and Grids events.
