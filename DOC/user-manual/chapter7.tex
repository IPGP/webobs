...
\begin{itemize}
	\item \wokey{FIELDSETxx\_NAME}: full name of the fieldset, but it is recommended to use short string since it will be used in the table column header,
...
\begin{itemize}
	\item \wokey{formula(size):\textit{formula}}, which allows to make simple mathematical operations between the different inputs and/or outputs (see below). Optional size of the result in characters (default is 6), and size$-3$ significant digits;
	\item \wokey{text} : text string (HTML allowed).
\end{itemize}

In formulas, the use of previous outputs is allowed but they must precede the current formula (with lower output numbers). The syntax must remain simple since the same formula is interpreted by Javascript in the form page and by Perl in the table page. Use of plus (+), minus (-), divide (/), multiply (*), exponent (**) operators, numbers and parenthesis is admitted. An input must be referenced as \wokey{INPUTzz}, an output as \wokey{OUTPUTzz}.
