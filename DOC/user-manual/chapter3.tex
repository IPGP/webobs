...
\subsubsection{General principle of SEFRAN3}
...
Signals are plotted from top to bottom of the graph in the order of the \wofile{MYSEFRAN\_Channels.conf} configuration file. Then the low-speed (\wokey{VALUE\_SPEED} inches per minute) image is exported as EPS file, converted to PNG using the resolution \wokey{VALUE\_PPI} (default is 100). A second high-speed image, using \wokey{VALUE\_SPEED\_HIGH} in/mn, is also exported as PNG.

\subsubsection{Spectrogram signal processing}

When \wokey{SGRAM\_ACTIVE} is on (default), a spectrogram is computed on each channels after applying an additional filter \wokey{SGRAM\_FILTER}, and normalization at peak-to-peak velocity amplitude. Parameters are (see also configuration file \ref{sefran3:conf}):
\begin{itemize}
\item \wokey{SGRAM\_FILTER}: signal filtering applied on all channels, same syntax as for waveform filters (default is \wokey{hpbu6,0.1} for a high-pass Butterworth 6th-order 0.1 Hz cut-off frequency filter);
\item \wokey{SGRAM\_PARAMS}: default parameters \textit{W, F\textsubscript{min}, F\textsubscript{max}, Y\textsubscript{scale}}, a 4-element vector of coma separated values, as:
	\begin{itemize}
		...
		\item \textit{Y\textsubscript{scale}} = \wokey{lin} for linear (default) or \wokey{log} (logarithmic).
	\end{itemize}
\item \wokey{SGRAM\_EXPONENT}: power spectrum amplitude exponent (default is 0.5 to reduce the contrast and visually amplify the low energy signal);
\item \wokey{SGRAM\_COLORMAP}: colormap (default is spectral);
\item \wokey{SGRAM\_CLIM}: color scale as 2-element vector as minimum, maximum values for colormap limits (default is 0-2, which corresponds to a saturation of the spectrogram colors when the signal is also saturated on the waveforms);
\item \wokey{SGRAM\_OPACITY}: initial opacity of spectrogram over waveform image (default is 0.7).
\end{itemize}

Default parameters \wokey{SGRAM\_PARAMS} values are optimized for 100 Hz sampling rate signals. It can be overwritten for any channel using the last column of \wofile{SEFRAN3\_Channels.conf}.  When the spectrogram is activated, minute and hourly images are made at low and high speed simultaneously with classical waveform images, and updated following broom wagon rules. The additional computing time is not significant. But, a spectrogram image are about 1.5 times bigger than waveform's images (low+high speed total).
...
SEFRAN3 is using extensively the network connection (for data flux) and external programs like \wofile{wget} for webservices like FDSN-dataselect, \wofile{arclink\_fetch} (from \textit{SeisComP3}), \wofile{slinktool} (from \textit{IRIS}). SEFRAN3 is also using extensively the programs \wofile{convert} (from \textit{ImageMagick}) and \wofile{pngquant} (from \url{https://pngquant.org}). If you experience any trouble, check first the network connection, the data availability on servers, and the third-party programs. You might add the variable \wokey{DEBUG|Y} in the configuration file to make the logs more verbose.
...
\lstinputlisting[title=\wofile{channels.conf}]{../../CODE/tplates/SEFRAN_Channels.conf}
\label{sefran3:channels}

\lstinputlisting[title=\wofile{SEFRAN template}]{../../CODE/tplates/SEFRAN.DEFAULT}
\label{sefran3:conf}
...
Interval value must be equal to (or lower than) \wokey{MIN\_RUNTIME\_SECONDS} to have continuous, real-time run. After activation, check that it runs correctly in the Scheduler Runs page (see Section \ref{scheduler}).
...
\lstinputlisting[title=\wofile{GENPLOT template}]{../../CODE/tplates/PROC.GENPLOT}
...
\item one optional summary graph \wokey{SUMMARY} as time series combining all \wo{nodes} on the 3 component subplots;
\item one optional summary graph \wokey{BASELINES} as time series computing 1-D distance variation between a selection of node pairs;
\item one optional summary graph \wokey{VECTORS} as map of velocity vectors and amplitude vs. distance;
\item one optional summary graph \wokey{MOTION} as map of relative displacements vs. time;
\item one optional summary graph \wokey{MODELLING} showing the potential source of deformation, estimating the location, volume variation / flux rate and source shape as result of point-source inversion in elastic medium (source type is isotropic or complex source pCDM);
...
\item \textbf{Grid:} turns on/off grid lines on graphs.
\item \textbf{PPI:} resolution for PNG output images, in pixel per inch.
\item \textbf{PDF:} outputs a PDF version of graphs (vectorial).
\item \textbf{SVG:} outputs a SVG version of graphs (experimental).
\item \textbf{Data exports:} outputs data (text files or others depending on the PROC).
\item \textbf{Anonymous:} checks to remove nominative subtitle (user's name).
\item \textbf{Verbose logs:} adds more information in logs (debug mode).
\end{itemize}

After submit the request, each of the PROC will be submitted to SCHEDULER as specific jobs. The name of the job is made from date and time of the request, hostname and user login name. Besides the Submit button, a check box \textit{Continue with this window (Replay!)} will stay on the form in order to keep parameters for another request. Without this box checked, the navigator will move automatically to the Results page.

If a job ends with success, a notification email is sent to the user through the POSTBOARD. The email provides two links: a first to access the request results (graphs and exports) through a web interface similar to the routine PROC graphs. The second link allows to download a .tgz archive containing all images and files of the request.

A dedicated page is also available to access request results: \wocmd{/cgi-bin/showREQ.pl}. The page will show the user's requests and results (if the request job has ended successfully), or all the existing requests for ADMIN users. The run of each PROC can be also followed on on the scheduler runs page. 
...
presented to the user in front of the full list of parameters accessible by the '+' icon. Parameters are filled by the proc's values and can be modified for request execution. There is no check of content validity, so any mistake might lead to request execution error.
...
This format uses a preprocessor, a script defined in \wofile{WEBOBS.rc} as \wokey{ROOT\_PREPROCESSOR}, as a filter of the rawdata file to output a regular semi-colon separated columns. The default preprocessor is a gawk script named \wofile{CODE/bin/preprocessor/dsv\_generic}.
