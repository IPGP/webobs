...
Each \wo{form} is then associated to a table in a database called \wofile{WEBOBSFORMS.db}. Each table of the database use the scheme shown in table \ref{table:genform_dbtable}.
...
\begin{table}[h]
	...
	\begin{tabular}{c c l}
		...
		{\tt edate}      & udate    & universal date and time end of measurement/collect/sampling \\
		{\tt sdate}      & udate    & universal date and time of optional start of measurement \\
		{\tt operators}  & text     & WebObs UID or GID of the operators (authorized list) \\
		{\tt comment}    & text     & \\
...
\begin{table}[h]
	...
	\begin{tabular}{c c l}
		...
		\hline
		{\tt id}         & integer  & primary key autoincrement\\
		{\tt date}       & text     & date and time (yyyy-mm-dd HH:MM:SS) \\
		{\tt date\_min}   & text     & uncertainty date and time (minimum value) \\
		{\tt yce}        & real     & years CE \\
		{\tt yce\_min}    & real     & uncertainty on years CE (minimum value) \\
		\hline
	\end{tabular}
	\caption{Scheme of the GENFORM database universal date (udate) table.}
	\label{table:genform_dbudate}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabular}{c c l}
		...
		\hline
		{\tt id}              & integer  & primary key autoincrement\\
		{\tt latitude}        & real     & decimal latitude (positive to North) \\
		{\tt northern\_error}  & real     & northern error on latitude (m) \\
		{\tt longitude}       & real     & decimal longitude (positive to East) \\
		{\tt eastern\_error}   & real     & northern error on longitude (m) \\
		{\tt elevation}       & real     & elevation above sea level (m) \\
		{\tt elevation\_error} & real     & error on elevation (m) \\
		\hline
	\end{tabular}
	\caption{Scheme of the GENFORM database geographic location (geoloc) table.}
	\label{table:genform_dbgeoloc}
\end{table}
...
To create a new \wo{form} using GENFORM, you have to go in the \wo{grid} menu list and click on the pencil next to \textbf{Raw Data}. Then, you have to choose a name for the FORM and select a template that will be used to help you through the creation process. Currently, several templates exist:

\begin{itemize}
	\item GENFORM: the most basic template that shows you all the different types of inputs that you can use in GENFORM (demo)
	\item WATERS: a template based on the former EAUX \wo{form} used for water chemistry analysis,
	\item VOLCGAS: a template based on the former GAZ \wo{form} used for gas chemistry analysis (exists also in French),
	\item BOJAP: a template based on the former BOJAP \wo{form} used for gas ’Japanese box’ (KOH) analysis,
	\item EXTENSO: a template based on the former EXTENSO \wo{form} used for extensometer measurements (exists also in French),
	\item FISSURO: a template based on the former FISSURO \wo{form} used for fissurometry measurements,
	\item RAINWATER: a template based on the former RAINWATER \wo{form} used for rain water chemical analysis,
	\item RIVERS: a template based on the former RIVERS \wo{form} used for river water chemical analysis,
	\item SOILSOLUTION: a template based on the former SOILSOULTION \wo{form} used for soil physico-chemical analysis,
\end{itemize}

\begin{figure}[!h]
	\centering
	\includegraphics[width=\textwidth]{figures/GENFORM_creation.png}
	\caption{To create a form, click on the plus icon, choose a FORM template in the list and enter a short name.}
