...
<script type="text/javascript">
	...
	function submit() {
		document.getElementById('geom').disabled=true;
		document.getElementById('geom').value = out;
		alert('salut');
	}
	...
	var editableLayers = new L.FeatureGroup();
		...
		map.on(L.Draw.Event.CREATED, function(e) {
		  ...
		  \/\*
		  \$.ajax('geomNODE.pl?geom='+out, {
		  	type: 'GET',
		  	data: {
		  		output: out
		  	},
		  	success: function(data) {
		  		console.log('geomNODE.pl called successfully');
		  	},
		  	error: function(jqXHR, textStatus, err) {
		  		console.error('geomNODE.pl call failed');
		  		console.error(err);
		  	}
		  });\*\/
		});
...
print "<form action='geomNODE.pl' method='get' onsubmit=\"document.getElementById('geom').value=out; window.close()\">";
