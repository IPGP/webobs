      ...
      foreach $duration (@durations) {
        ...
        if ($dur > $val) {
          ...
          $unite = $key;
			...
			my $newline = "$mc_id|$evt_sdate|$evt_stime|$type|$amp|$evt_dur|$unite|$duree_sat|$nb_evts|$evt_SP|$station|$is_unique|$sefran3_name|$evt_id|$img|$oper|$comment\n";
			print "$newline\n";
			push(@lignes,$newline);
		}
...
sub mc3_update {
  my ($mc3,$s3,$oper,$mcIDid,$date,$type,$amp,$dur,$duree_sat,$nb_evts,$evt_SP,$station,$is_unique,$evt_id,$img,$comment) = @_;
  ...
  if (defined($mcy)) {
    ...
    if (-e $mc_file)  {
      ...
      while(<FILE>) {
        ...
        chomp($line2);
        ($mc_id) = split(/\|/,$line2);
        my ($mcIDid_0,$date_0,$heure_0,$type_0,$amp_0,$duree_0,$unite_0,$duree_sat_0,
            $nb_evts_0,$evt_SP_0,$station_0,$is_unique_0,$sefran3_name_0,$evt_id_0,$img_0,$comment_0) = split(/\|/,$line2);
# --- TODO
# check we read correctly all the existing fields
# if new field is empty '', do not replace existing field
        # Ignore blank lines
        if (defined($mc_id)) {
          if ($mc_id == $mcIDid) {
            my @txt = split(/\|/,$line);
