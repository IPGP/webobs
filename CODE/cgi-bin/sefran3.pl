...
#DL-was:print " style=\"cursor:crosshair\" onMouseMove=\"ptr=flypointit(event,false);overlib(ptr,WIDTH,180,OFFSETX,0,FULLHTML)\" onClick=\"flypointit(event,true)\"";
    ...
    for (reverse @mc_hlist) {
        ...
        if (($MC{id} > 0 || ($userLevel == 4 && $trash == 1)) && $userLevel >= 1 && (defined $MC{minute} && ($MC{minute} - $Mc) <= $date_nbm)) {
            ...
            if ($MC{id} != $id) {
                print "<DIV class=\"mctag\" style=\"background-color:$types{$MC{type}}{Color};width:$dur_evt;height:$hauteur_image;left:$deb_evt;cursor:pointer\" 
                    onMouseOut=\"nd()\" onMouseOver=\"overlib('".js($MC{info})."',CAPTION,'$MC{firstarrival}',BGCOLOR,'$types{$MC{type}}{Color}',WIDTH,250)\"",
