...
my $domain = checkParam($cgi->param('domain'), qr/^[a-zA-Z0-9_-]*$/, "domain")
			// '';
...
if (! -e $gridConfFile) {
	...
	if ( open(FILE,">$gridConfFile") ) {
		...
		my $q = "insert into $WEBOBS{SQL_TABLE_GRIDS} VALUES(?, ?, ?)";
		my $rows = $dbh->do($q, undef, $GRIDType, $GRIDName, $domain);
		if (!$rows || $rows == 0) {
			htmlMsgDBError($dbh, "postGRID.pl: unable to insert grid"
							  ." $GRIDType.$GRIDName");
			exit;
		}
		$dbh->disconnect();
...
if (!sysopen(FILE, "$gridConfFile", O_RDWR | O_CREAT)) {
	...
	htmlMsgNotOK("postGRID.pl: error opening $gridConfFile: $!");
	exit;
}
unless(flock(FILE, LOCK_EX|LOCK_NB)) {
	warn "postGRID.pl: waiting for lock on $gridConfFile...";
	flock(FILE, LOCK_EX);
}
