...
function sind(x) { return Math.sin(dtr * x); }

//Cosine of angles in degrees
function cosd(x) { return Math.cos(dtr * x); }

//Tangent of angles in degrees
function tand(x) { return Math.tan(dtr * x); }

//Truncate large angles
function trunc(x) { return 360 * (x / 360 - Math.floor(x / 360)); }
...
function pixelY(deg) { return (centerDegH - deg) / pixelDegH; }
