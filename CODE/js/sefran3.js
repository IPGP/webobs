$(document).ready(function() {
    ...
    if (typeof MECB === 'undefined') {
        ...
        $('.flypointit').mousemove(function(e) {
            ptr = flypointit(e, false);
            overlib(ptr, WIDTH, 120, OFFSETX, 0, FULLHTML);
        });
        $('.flypointit').click(function(e) {
            flypointit(e, true)
        });
    }
    // spectrogram slider
...
function flypointit(event, click) {
    ...
    if (!click) {
        ...
        if (event.ctrlKey && event.clientX >= MECB.MFC.offsetLeft - MECB.PROX && event.clientX <= MECB.MFC.offsetLeft + MECB.MFC.scrollWidth + MECB.PROX) {
            if (MECB.MFC.offsetLeft == SCB.PPI + 5) {
                MECB.MFC.style.left = window.innerWidth - MECB.MFC.scrollWidth - 30;
            } else {
                MECB.MFC.style.left = SCB.PPI + 5;
            }
        }
...
function maj_formulaire() {
    ...
    if (MECB.FORM.dureeEvenement.value != "") {
        switch (MECB.FORM.uniteEvenement.value) {
            case "s":
                duration = MECB.FORM.dureeEvenement.value;
                break;
            case "mn":
                duration = MECB.FORM.dureeEvenement.value * 60;
                break;
            case "h":
                duration = MECB.FORM.dureeEvenement.value * 3600;
                break;
            case "d":
                duration = MECB.FORM.dureeEvenement.value * 86400;
                break;
        }
        if (MECB.FORM.secondeEvenement.value != "") {
...
function predict_seismic_event(pse_root_conf, pse_root_data, pse_algo_filepath, pse_conf_filename, pse_tmp_filepath, datasource, slinktool_prgm) {
    ...
    if (verif_to_predict()) {
        ...
        return $.ajax({
            ...
            (Object.keys(JSONdata).forEach(function(key) {
                ...
                if (event_html !== null) {
                    ...
                    console.log("Need to add class:", key)
                }
            }));
        ...
        alert("MISSING INPUTS TO PREDICT");
    }
}
