...
function set_wodp(days, months, holidays, min, max) {
    $('input#gdate').wodp({
        ...
        holidays: holidays,
        onpicked: function(i) {
            if ($('input#gdate').data('wodpdesc') != "ranging") submitform($('input#gdate').data('wodpdesc'));
        },
    });
...
function showtip(event, title, text, color) {
    ...
    if (event.pageY + tipH + 10 > vH) $("#gzt-tip").css({
        'top': event.pageY - tipH - 10
    });
    if (event.pageX + tipW + 10 > vW) $("#gzt-tip").css({
        'left': event.pageX - tipW - 10
    });
}
...
function shortcuts(shortcut, inputel) {
    ...
    switch (shortcut) {
        ...
        case "currWeek":
            ...
            d1 = new Date();
            d1.setDate(d0.getDate() + 6 - (d0.getDay() + 6) % 7);
            break;
        default:
    }
    var ds = d0.getFullYear() + '-' + lz((d0.getMonth() + 1)) + '-' + lz(d0.getDate());
    if (d1 != 0) {
        ds += ',' + d1.getFullYear() + '-' + lz((d1.getMonth() + 1)) + '-' + lz(d1.getDate())
    }
...
function openPopup(el, id) {
    ...
    $('input[type!="button"],select', form).each(function() {
    ...
    $.each(gazette_usrV, function(val, text) {
        $('#overlay_form_article #UID').append($('<option></option>').val(val).html(text))
    });
    if (id != -1) { // editing an existing article 'id': populate form fields first
        $.get("/cgi-bin/Gazette.pl", {
            "getid": id
        }, function(d) {
            ...
            form.OTHERS.innerHTML = d.OTHERS;
            form.OTHERS.value = form.OTHERS.innerHTML;
            form.PLACE.innerHTML = d.PLACE;
            form.PLACE.value = form.PLACE.innerHTML;
            form.SUBJECT.innerHTML = d.SUBJECT;
            form.SUBJECT.value = form.SUBJECT.innerHTML;
            $("option", form.CATEGORY).prop("selected", false);
            $('option[value=' + d.CATEGORY + ']', form.CATEGORY).prop("selected", true);
            $.each(gazette_usrI, function(val, text) {
                $('#overlay_form_article #UID').append($('<option></option>').val(val).html("<i>" + text + "</i>"))
            });
            $("option", form.UID).prop("selected", false);
            //d.UID.split('+').forEach(function(item) { $('option[value='+item+']',form.UID).prop("selected",true)});
            d.UID.split('+').forEach(function(item) {
                $('option[value="' + item + '"]', form.UID).prop("selected", true)
            });
...
function sendPopup() {
    ...
    var f0 = $("#overlay_form_article").serialize().split("&");
    var f1 = [];
    var uids = "";
    for (var i = 0; i < f0.length; i++) {
        if (f0[i].match(/^UID=/i)) {
            uids += "%2B" + (f0[i].replace(/UID=/, ""));
        } else {
            f1.push(f0[i]);
        }
    };
    ...
    var l0 = location.search.split("&");
    var l1 = [];
    for (var i = 0; i < l0.length; i++) {
        if (l0[i].match(/gview|gdate|gcategory|gfilter|wodpdesc/)) l1.push(l0[i]);
    }
...
function validPopup() {
    ...
    if (!form.STARTDATE.value.match(/^\d{4}-[0-1]\d-[0-3]\d$/)) {
        bad = true;
        form.STARTDATE.style.background = 'red';
    }
    if (form.ENDDATE.value != "") {
        if (!form.ENDDATE.value.match(/^\d{4}-[0-1]\d-[0-3]\d$/)) {
            bad = true;
            form.ENDDATE.style.background = 'red';
        }
    ...
    if (form.UID.value == "") {
        bad = true;
        form.UID.style.background = 'red';
    }
    if (form.CATEGORY.value == "") {
        bad = true;
        form.CATEGORY.style.background = 'red';
    }
    if (bad) return false;
    return true;
}
