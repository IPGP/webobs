(function($) {
    ...
    $.wodp = function(dominput, options) {
        ...
        ME.fill = function(arg) {
            ...
            if (arg == undefined) {
                if (ME.selranging == "") {
                    ...
                    ME.sel = {
                        from: new Date(inputvals[0]),
                        to: new Date(inputvals[1])
                    }
                    if (isNaN(ME.sel.from.getDate())) {
                        if (inputvals != "") {
                            ME.settings.oninvalid.call();
                        }
            ...
            for (var d = 0; d <= 41; d++) { // 42 days from & including tDate
                ...
                if (uDate.getDay() == 1) {
                    var ws = new Date(uDate.getWeek().wstart);
                    var we = new Date(uDate.getWeek().wend);
                    calbody += '<tr><th';
                ...
                for (var i = 0; i < ME.holidays.length; i++) {
                    if (uDate.getWODate() == ME.holidays[i].d) {
                        calbody += 'off';
                        title = ME.holidays[i].t;
                    }
                }
            ...
            caltable.css({
                top: $dominput.position().top + ME.settings.offY,
                left: $dominput.position().left + ME.settings.offX
            });
            caltable.find('tbody.wodpBody').append($(calbody));
            caltable.find('th, td').css('cursor', 'default');
            var m0 = ME.from.getWODate();
            var m1 = ME.to.getWODate();
            caltable.find('th.wodpPrevM').on('click', function(e) {
                ME.clickPrevMonth(e)
            }).css('cursor', 'pointer').attr('title', '> ' + m0);
            caltable.find('th.wodpNextM').on('click', function(e) {
                ME.clickNextMonth(e)
            }).css('cursor', 'pointer').attr('title', '< ' + m1);
            caltable.find('th span.wodpIdM').on('click', function(e) {
                ME.clickThisMonth(e)
            }).css('cursor', 'pointer');
            caltable.find('th span.wodpIdY').on('click', function(e) {
                ME.clickThisYear(e)
            }).css('cursor', 'pointer');
            caltable.find('th.wodpPrevY').on('click', function(e) {
                ME.clickPrevYear(e)
            }).css('cursor', 'pointer').attr('title', '> ' + m0);
            caltable.find('th.wodpNextY').on('click', function(e) {
                ME.clickNextYear(e)
            }).css('cursor', 'pointer').attr('title', '< ' + m1);
            if (ME.selranging == "") {
                caltable.find('[onclick]').css('cursor', 'pointer')
            } else {
                caltable.find('[onclick]').css('cursor', 'ew-resize')
            }
        ...
        ME.clickDay = function(event, day) {
            ...
            if (event.ctrlKey) {
                if (ME.selranging != "") {
                    if (ME.selranging < day) {
                        $dominput.val(ME.selranging + ',' + day)
                    } else {
                        $dominput.val(day + ',' + ME.selranging)
                    }
        ...
        ME.clickNextMonth = function(event) {
            ME.clickMonth(event, +1);
        }

        ME.clickPrevMonth = function(event) {
            ME.clickMonth(event, -1);
        }
        ...
        ME.clickNextYear = function(event) {
            ME.clickYear(event, +1);
        }

        ME.clickPrevYear = function(event) {
            ME.clickYear(event, -1);
        }
        ...
        ME.clickYear = function(event, way) {
            ...
            if (t.isInRange(ME)) {
                ...
                if (way > 0) {
                    ME.ref.setTime(ME.to.getTime())
                } else {
                    ME.ref.setTime(ME.from.getTime())
                }
            }
            $dominput.next('table.wodp').remove();
            ME.fill(ME.ref.getWODate());
        ...
        function buildHolidays(year) {
            ...
            if (ME.holidaysY != year) {
                ...
                for (var i = 0; i < ME.settings.holidays.length; i++) {
                    ME.holidays[i] = {
                        d: ME.settings.holidays[i].d,
                        t: ME.settings.holidays[i].t
                    };
                    var m = ME.settings.holidays[i].d;
                    if (m.match(/\$Y/)) {
                        ME.holidays[i].d = m.replace(/\$Y/, year);
                    }
                    if (m.match(/\$PQ$/)) {
                        ME.holidays[i].d = ES.getWODate();
                    }
                    if (m.match(/\$PQ (\d*)$/)) {
                        ME.holidays[i].d = m.replace(/\$PQ (.*)$/,
                            function(a, b) {
                                var s = new Date(ES);
                                s.setDate(s.getDate() + parseInt(b, 10));
                                return s.getWODate()
                            })
                    };
                }
