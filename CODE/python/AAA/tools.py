...
def print_metrics(dict, labels, metrics):
    """
    Pretty display for classification report
    """
    columnwidth = max([len(x) for x in metrics] + [5])
    bigcolumnwidth = columnwidth + 3
    empty_cell = " " * bigcolumnwidth
    # Print header
    print(empty_cell*2 + 'Metrics')
    print("    " + empty_cell, end=" ")
    for metric in metrics :
        print("%{0}s".format(columnwidth) % metric, end=" ")
    print()
    # Print rows
    for label in labels:
        print("    %{0}s".format(bigcolumnwidth) % label, end=" ")
        for metric in metrics:
            cell = "%{0}.3f".format(columnwidth) % dict[label][metric]
            print(cell, end=" ")
        print()
