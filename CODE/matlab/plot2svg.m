...
%  31-05-2018 - Added some latex characters like \copyright
...
if latex==1
    ...
    tex=regexprep(tex,'{\\fontsize{([0-9]*)}(.*)}','<tspan style="font-size:$1pt">$2</tspan>');
...
fprintf(fid,'      <text x="%0.3f" y="%0.3f" font-family="%s" text-anchor="%s" font-size="%0.0fpt"%s%s fill="%s" >', 0, 0, textfontname, anchor, fontsize, fweight, fangle, font_color);
fprintf(fid,'%s',tex);
fprintf(fid,'</text>\n'); 
fprintf(fid,'    </g>\n');
fprintf(fid,'  </g>\n');
