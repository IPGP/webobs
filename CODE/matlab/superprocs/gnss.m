		...
		if isok(P,'EXPORTS')
			...
			if any(~isnan(m0))
				E.infos = { ...
					...
					' ', ...
                    sprintf('Model space bounding box:'), ...
                    sprintf('  Eastern (UTM) = %1.0f m, %1.0f m',round(e0+minmax(xx))), ...
                    sprintf('  Northern (UTM) = %1.0f m, %1.0f m',round(n0+minmax(yy))), ...
                    sprintf('  Elevation (asl) = %1.0f m, %1.0f m',round(minmax(zz))), ...
					' ', ...
				};
				switch lower(mt)
				case 'pcdm'
