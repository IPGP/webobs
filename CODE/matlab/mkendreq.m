...
% request directory
reqdir = regexprep(regexprep(P.OUTDIR,sprintf('%s/',WO.ROOT_OUTR),''),sprintf('/%s',grid),'');

% root URL
url = field2str(P,'ORIGIN',field2str(WO,'ROOT_URL','http://webobs','notempty'),'notempty');
...
notify(WO,'formreq.','!',sprintf('uid=%s file=%s',P.UID,f));
