	...
	if exist(fdat,'file') && exist(fcdat,'file')
		...
		if ~isempty(dd)
			...
			k = find((t >= F.datelim(1) & t <= F.datelim(2)) ...
				& (isnan(dd(:,7))  | isinto(dd(:,7), P.LATLIM)) ...
				& (isnan(dd(:,8))  | isinto(dd(:,8), P.LONLIM)) ...
				& (isnan(dd(:,9))  | isinto(dd(:,9), P.DEPLIM)) ...
				& (isnan(dd(:,10)) | isinto(dd(:,10),P.MAGLIM)) ...
				& (isnan(dd(:,17)) | isinto(dd(:,17),P.MSKLIM)) ...
			);
