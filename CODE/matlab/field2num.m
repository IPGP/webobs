	...
	if ischar(val)
		...
		elseif ~isempty(regexp(f,'_(DATE|TLIM)$','once')) && ~isempty(val)
			try
				...
				try
					...
					wolog('** WARNING ** cannot convert %s key value (%s) to datenum...\n',f,val);
					y = sstr2num(val);
				end
			end
		...
		elseif ~isempty(regexp(f,'COLORMAP$','once'))
			if ~isempty(regexpi(val,'\.cpt$','once')) && exist(val,'file')
				...
				y = sstr2num(val);
				if size(y,2) ~= 3
					wolog('** WARNING ** cannot import %s = "%s" colormap. Use defaut...\n',f,val);
					if nargin > 2
						y = y0;
					else
						y = spectral(256);
		...
		elseif ~isempty(regexp(f,'_ALPHA$','once'))
			...
			if numel(y) < 2 || any(~isinto(y,[0,1]))
				wolog('** Warning: unvalid %s value (%s). Using default (0,1).\n',f,val);
				y = [0,1];
			end
