...
for si = 1:nx
    ...
    if mat & exist(f_save,'file')
		...
		if ~isnan(tlv(i))
		    ...
		    disp(sprintf('- Dernière mesure voie n°%d : unknown',i))
		end
        end
        tdeb = rmin(tlv);
        if isnan(tdeb)
    ...
    for iv = 0:2
        ...
        if ~isempty(find(~isnan(d)))
            ...
            G.inf = {sprintf('Dernière mesure: {\\bf%s} {\\it%+d}',datestr(tlv(iv+1)),G.utc), ...
...
G.tit = gtitle(stitre,'24h');
G.eta = [t2,etat,acqui];
G.inf = {'Dernière mesure:',sprintf('{\\bf%s} {\\it%+d}',datestr(t2),G.utc),' ',' '};
for i = 1:nx
