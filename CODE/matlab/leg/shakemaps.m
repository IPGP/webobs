...
for i = i_loop
    ...
    if (~exist(ftxt,'file') | test) & ~isempty(find(DH.typ(i) == tsok))
        ...
        % on fabrique également le commnuniqué si le code msk est supérieur ou égal à 2 - JMS
...
%                     unix(sprintf('mail %s -s "Séisme %s MD=%s - B3=%s (%s max) à %s" < %s',X.SISMO_EMAIL,dtiso,s_mag,msk2str(vmsk(kepi,2)),msk2str(vmsk(kepi,3)),s_gua,mtmp));
        ...
        % Création du message GSE si réellement ressenti : msk >= 2
        if DH.msk(i) >= 2
            fid = fopen(fgse,'wt');
            ...
            fprintf(fid,'Guadeloupe : %s %s à %s %s de %s (Guadeloupe)\n',s_qua,s_typ,s_epi,s_vaz,s_gua);
            fprintf(fid,'EVENT %s\n',fnam);
