...
t0 = datenum(2008,06,24,03,40,0);
t1 = datenum(2008,10,26,11,5,0);
...
ylabel('# Journalier')
colormap(copper)
set(gca,'XLim',[t(1)-1,t(end)+1],'TickDir','out','TickLength',[0.005,0.01],'LineWidth',.2)
datetick('x',10)
...
subplot(4,2,[3,5,7]);
pos = get(gca,'Position');
plot(tj - t0,dj(:,1),'LineWidth',.2), hold on
plot(tj - t0,mavr(dj(:,1),10),'-m','LineWidth',1)
%plot(th - t0,dh,'r')
hold off
set(gca,'Position',[pos(1)-0.08,pos(2),pos(3)+0.13,pos(4)],'LineWidth',.2)
...
%legend('Journalier','Journalier filtré (10 j)',1)

% Loi Omori 21/11/2004
subplot(8,2,6:2:10)
plot(th - t0,dh*24,'.r','MarkerSize',3)
hold on
plot(tj - t0 + 1,dj(:,1),'LineWidth',.2)
plot(tm - t0,dm/30,'-','LineWidth',1,'Color',[0,.7,0])
hold off
set(gca,'XScale','log','YScale','log','FontSize',8,'XLim',[1e-2,1e3],'YLim',[1,1e4],'linewidth',.2)
xlabel(sprintf('Temps écoulé depuis le %s (en jours)',datestr(t0)))
...
f = sprintf('%s/%s_HYPO.dat',pdon,rcode);
fid = fopen(f,'wt');
fprintf(fid,'# Fichier Hypocentres: date, heure et localisation des événements dans la zone des Saintes\n');
%fprintf(fid,'#AAAA-MM-DD hh:mm:ss LAT LON DEP MD ERH ERZ\n');
fprintf(fid,'#AAAA-MM-DD hh:mm:ss LAT LON DEP MD ERH\n');
for i = 1:length(k)
    %fprintf(fid,'%4d-%02d-%02d %02d:%02d:%05.2f %2.4f %2.4f %5.1f %3.1f %1.1f %1.1f\n',datevec(DH.tps(k(i))),DH.lat(k(i)),DH.lon(k(i)),DH.dep(k(i)),DH.mag(k(i)),DH.erh(k(i)),DH.erz(k(i)));
    fprintf(fid,'%4d-%02d-%02d %02d:%02d:%05.2f %2.4f %2.4f %5.1f %3.1f %1.1f %1.1f\n',datevec(DH.tps(k(i))),DH.lat(k(i)),DH.lon(k(i)),DH.dep(k(i)),DH.mag(k(i)),DH.erh(k(i)));
end
fclose(fid);
disp(sprintf('Fichier: %s créé.',f));
...
title(sprintf('Les Saintes: Histogramme des magnitudes du %s au %s',datestr(t0),datestr(tlim(2))),'FontWeight','bold')
...
set(gca,'XLim',tlim-t0,'FontSize',8,'lineWidth',.2)
...
%plot(tlim-t0,bv(1)*([1,1]),'--k','LineWidth',.2)
...
set(gca,'XLim',xylim(1:2),'YLim',xylim(3:4),'FontSize',8,'linewidth',.2)
set(h,'MarkerSize',10)
...
for i = 1:sz
	...
	hold off
    set(h,'MarkerSize',10)
    set(gca,'XLim',xylim(1:2),'YLim',xylim(3:4),'FontSize',6,'linewidth',.2)
