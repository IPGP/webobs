function DOUT = lac(mat,tlim,OPT,nograph,dirspec)
%LAC   Traitement des données des Sources Thermales.
%       LAC sans option charge les données les plus récentes du FTP
%       et retrace tous les graphes pour le WEB.
%
%       LAC(MAT,TLIM,OPT,NOGRAPH) effectue les opérations suivantes:
...
%
%       DOUT = LAC(...) renvoie une structure DOUT contenant toutes les 
...
%
%   Auteurs: F. Beauducel + C. Dessert, OVSG-IPGP
%   Création : 2010-03-15
...
% Importation de toutes les donnees meteo SANNER depuis ydeb
if ~nograph
        METEO = meteo(1,[ydeb,1,1,0,0,0;tnow]);
end
...
for st = 1:nx
    stn = so(st);
    stitre = sprintf('%s: %s',ST.ali{stn},ST.nom{stn});
    ...
    for ig = ivg
		...
		'', ...
                sprintf('Niveau = {\\bf%g m}',d(ke,i_nv)), ...
                sprintf('pH = {\\bf%1.2f}',d(ke,i_ph)), ...
                sprintf('Cl^- = {\\bf%1.1f mmol/l}',d(ke,i_cl)), ...
                sprintf('SO_4^{--} = {\\bf%1.1f mmol/l}',d(ke,i_so4)), ...
        ...
        % Niveau
        subplot(12,1,1:3), extaxes
        plot(t(k),d(k,i_nv),'o-','Color',gris1,'LineWidth',.1,'MarkerSize',G.mks{ig},'MarkerEdgeColor','k','MarkerFaceColor',gris2)
        set(gca,'XLim',G.lim{ig},'FontSize',8,'YGrid','on')
        datetick2('x',G.fmt{ig},'keeplimits')
        ylabel('Niveau (m)')
        
        % pH
        subplot(12,1,4:6), extaxes
        plot(t(k),d(k,i_ph),'o-','Color',gris1,'LineWidth',.1,'MarkerSize',G.mks{ig},'MarkerEdgeColor','k','MarkerFaceColor',gris2)
        set(gca,'XLim',G.lim{ig},'FontSize',8,'YGrid','on')
        datetick2('x',G.fmt{ig},'keeplimits')
        ylabel('pH')
        ...
        plot(t(k),d(k,i_cl),'o-','Color',gris1,'LineWidth',.1,'MarkerSize',G.mks{ig},'MarkerEdgeColor','k','MarkerFaceColor',gris2)
        set(gca,'XLim',G.lim{ig},'FontSize',8,'YGrid','on')
        datetick2('x',G.fmt{ig},'keeplimits')
        ylabel('Cl^- (mmol/l)')

        % Cl-/SO4--
        subplot(12,1,9:10), extaxes
        plot(t(k),d(k,23),'o-','Color',gris1,'LineWidth',.1,'MarkerSize',G.mks{ig},'MarkerEdgeColor','k','MarkerFaceColor',gris2)
        set(gca,'YScale','linear','XLim',G.lim{ig},'FontSize',8,'YGrid','on')
        datetick2('x',G.fmt{ig},'keeplimits')
        ylabel('Cl^-/SO_4^{--}')
	...
	ylabel(sprintf('Pluie Sanner (mm/j)'))
