...
pgamsk = [1,2,5,10,20,50,100,200,500,1000,2000]; % limites PGA équivalent échelle MSK (en milli g)
...
nomres = {'non ressenti';'très faible';'faible';'légère';'modérée';'forte';'très forte';'sévère';'violente';'extrême'};
nomdeg = {'aucun';'aucun';'aucun';'aucun';'très légers';'légers';'modérés';'moyens';'importants';'généralisés'};
txtb3 = sprintf('FB+CAH (c) OVSG-IPGP %s - Calculs basés sur localisation OVSG + loi d''atténuation B-Cube [Beauducel et al., 2004]',datestr(now,'yyyy'));
...
for i = 1:length(DH.tps)
    ...
    if (~exist(ftxt,'file') | test) & ~isempty(find(DH.typ(i) == tsok))
        ...
        fprintf(fid,'\n* Traitement automatique Loi d''atténuation B3 [Beauducel et al., 2004]\n');
        ...
        fprintf(fid,'* Distance hypocentrale et PGA calculé (estimation effets de sites = x %g):\n',efsite);
        ...
        if ress | test
            ...
            text(0,0,{'{\bfObservatoire Volcanologique et Sismologique de Guadeloupe - IPGP}', ...
                      ...
                      'Tél: +590 (0)590 99 11 33 - Fax: +590 (0)590 99 11 34 - infos@ovsg.univ-ag.fr - www.ipgp.jussieu.fr'}, ...
