function DOUT=rap(mat,tlim,OPT,nograph)
...
%           - exploitation des déclenchements fichiers SAC existants (utilise rsac.m)
...
G = readgr(rcode);
tnow = datevec(G.now);
ST = readst(G.cod,G.obs,0);		% lit toutes les stations
...
if mat == 0
    unix(sprintf('rm -rf %s/%s/*',pftp,pmap));
    delete(fpga);
    disp('Make all the maps and recompute PGA...');
end
...
DH.cod = [cse(k);DH.cod];
clear t d cse k
disp(sprintf('File: %s imported.',fhyp));
...
for i = 1:length(t)
    ...
    if isempty(k)
        ...
        if isempty(kk)
            disp(sprintf('Warning: data associated with unexistant station %s !',sit{i}));
        else    
        ...
        f_map = sprintf('%s/%s/%s/%s_rap.png',pftp,pmap,pam,datestr(DH.tps(k(1)),30));
    end
    ...
    if exist(f_ps,'file')
	    ...
	    if isempty(kk)
	    	...
	    	disp(sprintf('f_png = %s (exist = %d)',f_png,exist(f_png,'file')))
	    	disp(sprintf('f_map = %s (exist = %d)',f_map,exist(f_map,'file')))
			...
			[x1,h] = rsac(f1,'le');
			disp(sprintf('File: %s loaded.',f1))
			[x2,h] = rsac(f2,'le');
			disp(sprintf('File: %s loaded.',f2))
			% ===============================================================
			...
			%
			% calibration des signaux [Z,NS,EW] = Ch.Nb 1,2,3 dans les fichiers CLB
			[dc,C] = calib(t(i),[zeros(size(x1)),x1-mean(x1),x2-mean(x2)],ST.clb(kk));
			dx = complex(dc(:,3),dc(:,2));
			d(i,2) = max(abs(dx));
			if ~isnan(d(i,2))
				km = find(abs(dx)==d(i,2));
				d(i,3) = angle(dx(km(1)));
			else
				d(i,3) = NaN;
			end
			...
			fclose(fid);
			disp(sprintf('File: %s updated.',fpga));
		    end
		    ...
		    disp(sprintf('Graph:  %s created.',f_png));
	    end
        end
    else
        d(i,2) = NaN;
...
for i = 1:length(kh)
    ...
    f_map = sprintf('%s_rap',datestr(DH.tps(ki),30));
    if i == 1
	...
	% [FB] utilise la fonction attenuation.m qui intègre la loi B3
        xpga = attenuation(2,DH.mag(ki),xdhp);
        h1 = plot(xdhp,xpga,'-k','LineWidth',2);
        hold on
	plot(xdhp,xpga*3,'--k');
	h2 = plot(xdhp,xpga/3,'--k');
        kk = find(d(k,5)==1);
        h3 = plot([.9;d(k(kk),4)],[1e-6;d(k(kk),2)],'dk','MarkerFaceColor',Tsol{2}{1});
        kk = find(d(k,5)~=1);
        h4 = plot([.9;d(k(kk),4)],[1e-6;d(k(kk),2)],'dk','MarkerFaceColor',Tsol{2}{2});
        hold off
        set(gca,'XLim',dhplim,'YLim',pgalim,'Xscale','log','Yscale','log','FontSize',8)
        legend([h1,h2,h3,h4],[{'Loi B3'},{'incertitude B3'},Tsol{1}],3)
        grid on
...
fclose(fid0);  disp(sprintf('File: %s created.',fdon));
fclose(fid);  disp(sprintf('File: %s/%s created.',pwww,flst));
