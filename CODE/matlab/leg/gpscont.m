...
fxmin = 50;    % pourcentage minimum d'ambiguités fixées pour le tracé
ermax = 0.1;    % erreur maximum pour le tracé (en mètre)
st0 = 'SOUF'; % Station de référence
%veille = {'ASF','DDU','PDB'};
...
i0 = find(strcmp(ST.dat,st0));
so = [];

for i = 1:length(ST.dat)
    %if ~strcmp(ST.dat(i),veille) & ~strcmp(ST.dat(i),'-') & i ~= i0
    if ~strcmp(ST.dat(i),'-') & i ~= i0
        so = [so,i];
    end
end
nx = length(so);
...
% Importation du fichier de données GAMIT
f = sprintf('%s/base-GAMIT.dat',pftp);
...
for i = 1:length(ss)
    if length(ss{i}) > 10
        [y,j,de,sde,dn,sdn,du,sdu,dd,sdd,fx,am,tx1,tx2,tx3] = strread(ss{i},'%f%f%f%f%f%f%f%f%f%f%s%s%s%s%s%*[^\n]');
        t(i,1) = datenum([y 0 0 0 0 0]) + j;
        ii0 = find(strcmp(ST.dat,tx2));
        if isempty(ii0), ii0 = NaN; end
        ii = find(strcmp(cellstr(sdat(:,1:3)),tx3{1}(6:8)));
...
if nargout > 0
    for st = 1:length(ST.cod)
        k = find(strcmp(lower(slog),ST.dat) & flog > 0 & tlog>=G.lim{ivg(1)}(1) & tlog<=G.lim{ivg(1)}(2));
        DOUT(st).code = ST.cod;
        DOUT(st).time = tlog(k);
        DOUT(st).data = flog(k);
    end
...
if ivg(1) == 1
    sox = find(~strcmp(ST.dat,'-'));
    nxx = length(sox);
    etats = zeros(nxx,1);
    acquis = zeros(nxx,1);
    for i = 1:nxx
        st = sox(i);
        k = find(strcmp(slog,ST.dat(st)) & flog > 0);
        if ~isempty(k)
...
for st = 1:nx
    ...
    for ig = ivg
        ...
        if ~isempty(k)
            ...
            dke = dke - repmat(dke(1,:),[size(dke,1),1]);
            dkn = dkn - repmat(dkn(1,:),[size(dkn,1),1]);
            dkz = dkz - repmat(dkz(1,:),[size(dkz,1),1]);

            % définit l'échelle des ordonnées
            %v = .5*ceil(10*abs(diff(minmax(dk))))/10;
            ww = 2*ceil(10*max(std(dk) + .01))/10;
            %v(1) = 2*ceil(10*max(std(dke) + .01))/10;
            v(1) = round(40*max(std(dke)+0.01))/10;
			if v(1) < 0.01, v(1) = 0.01; end
            %v(2) = 2*ceil(10*max(std(dkn) + .01))/10;
            v(2) = round(40*max(std(dkn)+0.01))/10;
			if v(2) < 0.01, v(2) = 0.01; end
            %v(3) = 2*ceil(10*max(std(dkz) + .01))/10;
            v(3) = round(40*max(std(dkz)+0.01))/10;
            if v(3) < 0.01, v(3) = 0.01; end
