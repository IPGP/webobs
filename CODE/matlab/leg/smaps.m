...
for i = 1:nb
	if test
	    switch i-1
	    case 0 
		DH.tps(i) = datenum(2007,11,28,10,35,00); DH.lat(i) = 15.144; DH.lon(i) = -60.442; DH.dep(i) = 62; DH.mag(i) = 4.8; DH.typ(i) = 2;
	    case 1
		DH.tps(i) = datenum(2008,3,28,16,54,18); DH.lat(i) = 16.146; DH.lon(i) = -61.499; DH.dep(i) = 7.0; DH.mag(i) = 3.3; DH.typ(i) = 2;
	    case 2
		DH.tps(i) = datenum(2008,5,21,11,43,18); DH.lat(i) = 15.909; DH.lon(i) = -61.0081; DH.dep(i) = 72; DH.mag(i) = 3.9; DH.typ(i) = 2;
	    case 3
		DH.tps(i) = datenum(2008,2,6,18,37,58); DH.lat(i) = 15.0808; DH.lon(i) = -60.327; DH.dep(i) = 12; DH.mag(i) = 4.8; DH.typ(i) = 2;
	    otherwise
	    end
    end
    % ==========================================================
...
%     if (~exist(ftxt,'file') | test) & ~isempty(find(DH.typ(i) == tsok))
	...
	vpga = attenuation(id_loi,DH.mag(i),vdhp,DH.dep(i));
        ...
        if ress | test
            ...
            if test
                text(.3,0,'Exercice','FontSize',72,'FontWeight','bold','Color',[0,0,1],'Rotation',15,'HorizontalAlignment','center');
            end
